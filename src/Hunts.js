...
export async function fetchHunt() {
  ...
  const resHunt = await fetch(`https://${process.env.REACT_APP_HUNT_API_URL}/hunt/1`);

  const hunt = await resHunt.json();

  const resLocations = await fetch(`https://${process.env.REACT_APP_HUNT_API_URL}/locations/1`);
